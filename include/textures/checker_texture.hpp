...
texture_color(checker_texture<EvenTexture, OddTexture> const &texture,
              texture_coord_t coord, point3 hit_point) {
  ...
  return is_even ? texture_color(texture.even_texture, coord, hit_point)
                 : texture_color(texture.odd_texture, coord, hit_point);
}
} // namespace texture
} // namespace mrl
