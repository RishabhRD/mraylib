...
void ramayan() {
  ...
  auto space_img = mrl::stb_image{"data/earthmap.jpg"};
  auto space_texture = image_texture{std::move(space_img)};

  diffuse_light hanuman{std::move(hanuman_texture)};
  diffuse_light ram{std::move(ram_texture), 4};
  diffuse_light garuna{std::move(garuna_texture)};
  diffuse_light jaishreeram{std::move(jaishreeram_texture)};
  lambertian_t ground{std::move(space_texture)};

  std::vector<any_object> world{
      shape_object{quad{mrl::point3(-8, -2, 4), mrl::vec3(-1, 0, -4),
                        mrl::vec3(0, 4, 0)},
                   std::move(hanuman)},
      shape_object{quad{mrl::point3(-9, -4, 0), mrl::vec3(18, 0, 0),
                        mrl::vec3(0, 10, 0)},
                   std::move(ram)},
      shape_object{
          quad{mrl::point3(8, -2, 1), mrl::vec3(0, 0, 4), mrl::vec3(0, 4, 0)},
          std::move(garuna)},
      shape_object{quad{mrl::point3(-3, -3, 5), mrl::vec3(6, 0, 0),
                        mrl::vec3(0, 0.7, -1)},
                   std::move(jaishreeram)},
      shape_object{sphere{50, mrl::point3(0, -53, 0)}, std::move(ground)},
  };
